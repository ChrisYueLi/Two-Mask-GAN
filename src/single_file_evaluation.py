...
def enhance_one_track(
    model, audio_path, saved_dir, cut_len, device, n_fft=400, hop=160, save_tracks=False
):
    ...
    for input_wave in waveforms:
        noisy = torch.from_numpy(input_wave).to(device).unsqueeze(0)
    ...
    #     noisy = torch.reshape(noisy, (batch_size, -1))
        ...
        noisy_spec = torch.view_as_real(torch.stft(
            noisy, n_fft, hop, window=torch.hamming_window(n_fft).to(device), onesided=True, return_complex=True
        ))
...
def evaluation(model_path, noisy_dir, save_tracks, saved_dir, device, num_channel, mask_mode, module):
    ...
    est_audio = enhance_one_track(
        model, audio, saved_dir, 1600 * 5, device, n_fft, 160, save_tracks
    )
